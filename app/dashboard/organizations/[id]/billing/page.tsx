...
export default function BillingDashboardPage() {
  ...
  const fetchPlans = async () => {
    try {
      ...
      logger.error('Error fetching plans', err instanceof Error ? err : undefined, { errorMsg: String(err) })
    }
  }

  const fetchRecentInvoices = async () => {
    try {
      const res = await fetch(`/api/organizations/${orgId}/billing/invoices?limit=5`)
      if (!res.ok) throw new Error('Failed to fetch invoices')
      const data = await res.json()
      setRecentInvoices(data.invoices || [])
    } catch (err) {
      logger.error('Error fetching invoices', err instanceof Error ? err : undefined, { errorMsg: String(err) })
    }
  }
  ...
  const handleReactivateSubscription = async () => {
    setActionLoading(true)
    try {
      const res = await fetch(`/api/organizations/${orgId}/billing/reactivate`, {
        method: 'POST',
      })
  ...
  const openChangePlanDialog = async (plan: SubscriptionPlan) => {
    ...
    if (billingData?.subscription?.stripe_subscription_id) {
      try {
        ...
        logger.error('Error calculating proration', err instanceof Error ? err : undefined, { errorMsg: String(err) })
      }
    }
  }
  ...
  const getBillingCycleProgress = () => {
    if (!billingData?.subscription?.current_period_start || !billingData?.subscription?.current_period_end) {
      return 0
    }
    const start = new Date(billingData.subscription.current_period_start).getTime()
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      <div className="space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              {billingData?.plan ? (
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={billingData.plan.is_featured ? 'default' : 'outline'}>
                      {billingData.plan.tier}
                    </Badge>
                  </div>
                  <div className="space-y-2">
                    <div className="text-3xl font-bold">
                      {formatPrice(billingData.plan.price_cents, billingData.plan.currency)}
                  ...
                  {billingData.subscription?.current_period_end && !billingData.subscription.cancel_at_period_end && (
                    <div className="text-sm text-muted-foreground">
                      Next billing date: {formatDate(billingData.subscription.current_period_end)}
                    </div>
                  )}
                  <div className="pt-2 space-y-2">
                    {billingData.subscription?.cancel_at_period_end ? (
                      ...
                      <Button onClick={handleOpenBillingPortal} disabled={actionLoading} className="w-full">
                        Manage Billing
                      </Button>
                    )}
                  </div>
          ...
          {/* Usage Overview Card */}
          {billingData?.subscription && billingData.subscription.status !== 'canceled' && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  <>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">{getDaysRemainingInPeriod()} days left</span>
                      </div>
                      <Progress value={getBillingCycleProgress()} className="h-2" />
                    </div>
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Started</span>
                        <span>{formatDate(billingData.subscription.current_period_start)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Ends</span>
                        <span>{formatDate(billingData.subscription.current_period_end)}</span>
                      </div>
                    </div>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {isCurrentPlan ? (
                        ...
                        <Button
                          ...
                          className="w-full"
                        >
                          {plan.price_cents > (billingData.plan?.price_cents || 0)
                            ? 'Upgrade'
                            : 'Downgrade'}
                        </Button>
                      ) : (
                        <Button
                          onClick={() => router.push(`/api/organizations/${orgId}/billing/checkout?plan_id=${plan.id}`)}
                          className="w-full"
                        >
                          Get Started
                        </Button>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => router.push(`/dashboard/organizations/${orgId}/billing/invoices`)}
                >
                  View All Invoices
                </Button>
              </div>
        ...
        {billingData?.subscription && !billingData.subscription.cancel_at_period_end && (
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">
                    You can cancel at any time. Your subscription will remain active until the end of the
      ...
      <Dialog open={changePlanDialog} onOpenChange={setChangePlanDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <p className="text-sm text-muted-foreground">
              Your plan will be updated immediately, and you'll be charged or credited a prorated amount
