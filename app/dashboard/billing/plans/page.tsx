...
export default function PricingPlansPage() {
  ...
  const { data: plans, loading, error, execute } = useAsync<SubscriptionPlan[]>()
  ...
  const fetchPlans = () => execute(async () => {
  ...
  const monthlyPlans = (plans || []).filter((p) => p.billing_interval === 'month')
  const _yearlyPlans = (plans || []).filter((p) => p.billing_interval === 'year')
      ...
      <div className="space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Flexible pricing for organizations of all sizes. Start free and scale as you grow.
          </p>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {displayPlans.map((plan) => {
            const displayPrice = yearlyBilling
              ...
              : plan.price_cents
            const displayInterval = yearlyBilling ? 'year' : plan.billing_interval
              ...
              >
                  ...
                  <div className="space-y-2">
                    ...
                    {yearlyBilling && plan.price_cents > 0 && (
                      <div className="text-sm text-muted-foreground">
                        {formatPrice(plan.price_cents, plan.currency)}/month billed yearly
                      </div>
                    )}
                  </div>
                ...
                <CardContent className="space-y-6 pt-6">
                  ...
                  <div className="pt-4 border-t space-y-2 text-sm">
                    <div className="font-medium text-muted-foreground">Usage Limits</div>
                    <div className="space-y-1 text-muted-foreground">
                      ...
                      {plan.max_members && <div>Up to {plan.max_members} members</div>}
                      {plan.max_api_keys && <div>Up to {plan.max_api_keys} API keys</div>}
                      {plan.max_webhooks && <div>Up to {plan.max_webhooks} webhooks</div>}
                      {plan.storage_gb && <div>{plan.storage_gb}GB storage</div>}
                    </div>
                  </div>
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                      ...
                      {displayPlans.map((plan) => (
                        <TableCell key={plan.id} className="text-center">
                          {plan.price_cents === 0
                            ? 'Free'
                            : formatPrice(
                                yearlyBilling ? getYearlyPrice(plan.price_cents) : plan.price_cents,
                                plan.currency
                              )}
                          {plan.price_cents > 0 && `/${yearlyBilling ? 'year' : 'month'}`}
                        </TableCell>
                      ))}
                    </TableRow>
                    <TableRow>
                    ...
                    <TableCell className="font-medium">Requests per hour</TableCell>
                    {displayPlans.map((plan) => (
                      <TableCell key={plan.id} className="text-center">
                    ...
                    <TableRow>
                      <TableCell className="font-medium">Requests per day</TableCell>
                      {displayPlans.map((plan) => (
                        <TableCell key={plan.id} className="text-center">
                    ...
                    <TableRow>
                      <TableCell className="font-medium">Max API keys</TableCell>
                      {displayPlans.map((plan) => (
                        <TableCell key={plan.id} className="text-center">
                          {plan.max_api_keys === null ? 'Unlimited' : plan.max_api_keys}
                        </TableCell>
                      ))}
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">Max webhooks</TableCell>
                      {displayPlans.map((plan) => (
                        <TableCell key={plan.id} className="text-center">
                          {plan.max_webhooks === null ? 'Unlimited' : plan.max_webhooks}
                        </TableCell>
                      ))}
                    </TableRow>
                    <TableRow>
                      ...
                      {displayPlans.map((plan) => (
                        <TableCell key={plan.id} className="text-center">
                          {plan.storage_gb === null ? 'Unlimited' : `${plan.storage_gb}GB`}
                        </TableCell>
                      ))}
                    </TableRow>
                    <TableRow>
        ...
        <div className="text-center space-y-4 py-12 border-t">
          <div className="flex items-center justify-center gap-2 text-muted-foreground">
            ...
            <p>All plans include 256-bit SSL encryption and SOC 2 Type II compliance</p>
          </div>
          <p className="text-sm text-muted-foreground">
            Have questions? Contact our sales team at{' '}
            <a href="mailto:sales@neuroelemental.com" className="text-primary hover:underline">
