...
export const GET = createAuthenticatedRoute<{ id: string }>(
  async (request, context, user) => {
    ...
    const response: {
      credits: Awaited<ReturnType<typeof getOrganizationAllCredits>>;
      transactions?: Awaited<ReturnType<typeof getCreditTransactions>>;
      statistics?: Awaited<ReturnType<typeof getCreditStatistics>>;
    } = { credits };
    ...
    if (includeTransactions) {
      ...
      const credit_type = searchParams.get("credit_type") as
        | CreditType
        | undefined;
...
export const POST = createAuthenticatedRoute<{ id: string }>(
  async (request, context, user) => {
    ...
    const { credit_type, amount, payment_id, expiration_days, metadata } = body;
