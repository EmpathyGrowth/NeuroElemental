...
import { createAuthenticatedRoute, forbiddenError, notFoundError, badRequestError, successResponse } from '@/lib/api'
import { isUserOrgMember, isUserOrgOwner } from '@/lib/db'
import { getOrganizationRole, updateOrganizationRole, deleteOrganizationRole, getMembersCountByRole } from '@/lib/permissions/manage'
 ...
 */
export const PATCH = createAuthenticatedRoute<{ id: string; roleId: string }>(async (request, context, user) => {
  const { id, roleId } = await context.params

  // Check if user is organization owner
  const isOwner = await isUserOrgOwner(user.id, id)
  if (!isOwner) throw forbiddenError('Only organization owners can update roles')
 ...
 * Requires: Organization owner
 */
export const DELETE = createAuthenticatedRoute<{ id: string; roleId: string }>(async (_request, context, user) => {
  const { id, roleId } = await context.params

  // Check if user is organization owner
  const isOwner = await isUserOrgOwner(user.id, id)
  if (!isOwner) throw forbiddenError('Only organization owners can delete roles')
